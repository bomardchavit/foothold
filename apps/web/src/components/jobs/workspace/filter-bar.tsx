...
import { Lock, MoreHorizontal, HelpCircle } from "lucide-react";
...
export function FilterBar({ filters, industries, targetRoles, hiddenCount, lowQualityHidden }: { filters: FeedFilters; industries: string[]; targetRoles: string[]; hiddenCount: number; lowQualityHidden: number }) {
  ...
  const chip = (label: string, count?: number, active?: boolean) => (
    <span className={cn("inline-flex h-9 items-center gap-1.5 whitespace-nowrap rounded-lg px-3.5 text-[14px] transition-colors", active ? "bg-accent font-medium" : "bg-muted hover:bg-accent/70")}>
      {label}{count ? <span>(+{count})</span> : null}<span aria-hidden className="ml-0.5 text-[10px] leading-none text-foreground/70">▼</span>
    </span>
  );
  const roleOptions = [...new Set([...targetRoles, ...ROLE_SUGGESTIONS])];
  return (
    <div className="flex flex-wrap items-center gap-2.5 px-6 pt-5" data-testid="filter-rail">
      ...
      <button onClick={() => apply({ hidden: !filters.hidden })} data-testid="chip-hidden" className={cn("inline-flex h-9 items-center gap-1.5 rounded-lg px-3.5 text-[14px] transition-colors", filters.hidden ? "bg-primary/20 font-medium" : "bg-primary/10 hover:bg-primary/20")}>
        <Lock className="h-4 w-4" /> Hidden Jobs{hiddenCount ? ` (${hiddenCount})` : ""}
      </button>

      <AllFilters filters={filters} apply={apply} toggle={toggle} lowQualityHidden={lowQualityHidden} />

      <Tooltip><TooltipTrigger asChild><button className="text-foreground/60 hover:text-foreground" aria-label="How matching works"><HelpCircle className="h-5 w-5" /></button></TooltipTrigger><TooltipContent className="max-w-xs text-xs">Fit is six weighted components: skills, profile relevance, level, years, industry, location. Open any job to see the evidence for each.</TooltipContent></Tooltip>
...
function AllFilters({ filters, apply, toggle, lowQualityHidden }: { filters: FeedFilters; apply: (p: Partial<FeedFilters>) => void; toggle: (k: "h1b", v: string) => void; lowQualityHidden: number }) {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <button onClick={() => setOpen(true)} data-testid="chip-all" className="inline-flex h-9 items-center gap-1.5 rounded-lg bg-primary px-3.5 text-[14px] font-medium text-primary-foreground transition hover:brightness-110"><MoreHorizontal className="h-4 w-4" /> All Filters{active ? ` (${active})` : ""}</button>
