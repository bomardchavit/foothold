...
interface Src { id: string; kind: JobSourceKind; slug: string; name: string | null; enabled: boolean; jobs: number; lastRun: string | null; lastStatus: string | null; lastCounts: string | null; nextRun: string | null; nextRunOverdue: boolean; everyMin: number }
  ...
  return (
    <div className="space-y-6 text-sm">
      <div className="rounded-xl border bg-card p-4">
        ...
        <p className="mt-1 text-muted-foreground">{scheduler.mode === "queue" ? "The worker polls the boards whose turn has come, every ten minutes." : scheduler.everyMin ? `Every ${scheduler.everyMin} minutes the app polls the boards whose turn has come (SCRAPE_INTERVAL_MIN). A board with nothing new answers in one request and nothing is re-parsed.` : "In-app scheduling is off (SCRAPE_INTERVAL_MIN=0); run sources by hand or start the worker."} Jobs outside <code>{countries}</code> are skipped (JOBS_COUNTRIES). No keys are needed for any of this.</p>
        ...
        {sources.map((s) => (
          <li key={s.id} className="flex flex-wrap items-center justify-between gap-3 p-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">{s.jobs} jobs{s.lastRun ? ` · checked ${s.lastRun}: ${s.lastStatus ?? ""}${s.lastCounts && s.lastCounts !== s.lastStatus ? ` (${s.lastCounts})` : ""}` : " · never checked"}{s.enabled ? ` · every ${s.everyMin} min${s.nextRunOverdue ? ", due now" : s.nextRun ? `, next ${s.nextRun}` : ""}` : ""}</p>
            </div>
