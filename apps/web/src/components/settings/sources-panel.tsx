  ...
  return (
    <div className="space-y-6 text-sm">
      ...
      <form className="flex flex-wrap items-end gap-2 rounded-xl border bg-card p-4" onSubmit={(e) => { e.preventDefault(); start(async () => { const r = await addSourceAction({ kind, slug, name }); if (r.ok) { setSlug(""); setName(""); toast.success("Source added"); } else toast.error(r.error); }); }}>
        <div><label className="text-xs text-muted-foreground">Kind</label><select className="mt-1 block h-9 rounded-md border bg-background px-2" value={kind} onChange={(e) => setKind(e.target.value as JobSourceKind)}>{["GREENHOUSE", "LEVER", "ASHBY", "SMARTRECRUITERS", "WORKABLE", "WORKDAY", "CAREERS", "ADZUNA", "USAJOBS"].map((k) => <option key={k} value={k}>{k.toLowerCase()}</option>)}</select></div>
