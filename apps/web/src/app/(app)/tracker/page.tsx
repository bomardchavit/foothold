...
import { KanbanBoard, type BoardApplication } from "@/components/tracker/kanban-board";
...
export default async function TrackerPage() {
  ...
  const [apps, docs] = await Promise.all([
    ...
    prisma.resumeDocument.findMany({ where: { userId: user.id }, select: { id: true, title: true, kind: true, jobId: true }, orderBy: { createdAt: "desc" } }),
  ]);
  ...
  return (
    <div>
      ...
      <KanbanBoard applications={data} resumes={docs} />
    </div>
  );
}
