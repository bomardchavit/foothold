...
export default async function SourcesPage() {
  ...
  return (
    <SourcesPanel keys={{ adzuna: Boolean(env.adzuna), usajobs: Boolean(env.usajobs) }} sources={sources.map((s) => ({ id: s.id, kind: s.kind, slug: s.slug, name: s.name, enabled: s.enabled, jobs: s._count.jobs, lastRun: s.lastRunAt ? formatDistanceToNowStrict(s.lastRunAt, { addSuffix: true }) : null, lastStatus: s.lastStatus, lastCounts: s.runs[0] ? `${s.runs[0].fetched} fetched, ${s.runs[0].inserted} new, ${s.runs[0].updated} updated` : null }))} />
  );
}
